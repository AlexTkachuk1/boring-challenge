...
export class ReelManager {
    ...
    public initReels(): void {
        for (let x = 0; x < 3; x+=1) {
            for (let y = 0; y < 3; y+=1) {
                ...
                const image: Phaser.GameObjects.Image = this._scene.add.image(posX, posY, key).setDepth(1).setScale(0.33);
                const symbol: Symbol = {
                    x,
                    y,
                    startX: posX,
    ...
    public async onWin(): Promise<void> {
        ...
        const anim = this._scene.add.spine(508, 500, ArtKeys.Win, ArtKeys.BigWin, false).setDepth(3);
        anim.timeScale = 1.7;
